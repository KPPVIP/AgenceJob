...
Citizen.CreateThread(function()
	...
	RMenu.Add('menu', 'facture', RageUI.CreateSubMenu(RMenu:Get('menu', 'main'), "Intéraction Citoyens", "~b~Intéraction Citoyens"))
	RMenu.Add('menu', 'annonces', RageUI.CreateSubMenu(RMenu:Get('menu', 'main'), "Gestions Annonces", "~b~Choix des Annonces"))
    ...
    if A.Menu then
        ...
        Citizen.CreateThread(function()
			while A.Menu do
				...
				RageUI.Separator("↓ ~y~     ~s~↓")

				RageUI.ButtonWithStyle("Intéraction Citoyens", nil, {RightLabel = "→→"},true, function()
				end, RMenu:Get('menu', 'facture'))

				RageUI.ButtonWithStyle("Gestions des Annonces", nil, {RightLabel = "→→"},true, function()
				end, RMenu:Get('menu', 'annonces'))
			end)
		...
		RageUI.IsVisible(RMenu:Get('menu', 'annonces'), true, true, true, function()

			RageUI.ButtonWithStyle("~g~Annonces d'ouverture",nil, {RightLabel = ""}, true, function(Hovered, Active, Selected)
				if Selected then      
					TriggerServerEvent('OuvertA')
				end
			end)
	
			RageUI.ButtonWithStyle("~r~Annonces de fermeture",nil, {RightLabel = ""}, true, function(Hovered, Active, Selected)
				if Selected then     
					TriggerServerEvent('FermerA')
				end
			end)
		...
		RageUI.Separator("↓      ↓")
			...
			RageUI.ButtonWithStyle("~y~Annonce Début Recrutement",nil, {RightLabel = ""}, true, function(Hovered, Active, Selected)
				if Selected then     
					TriggerServerEvent('RecrutementAD')
				end
			end)

			RageUI.ButtonWithStyle("~y~Annonce Fin Recrutement",nil, {RightLabel = ""}, true, function(Hovered, Active, Selected)
				if Selected then     
					TriggerServerEvent('RecrutementAF')
				end
			end)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
		if IsControlJustPressed(0,167) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'realestateagent' then
			openPropertiesMenu() 
	  ...
	  function(data, menu)
		...
		if player ~= -1 and distance <= 3.0 then
		  ...
		  if amount == nil then
			...
			Citizen.Wait(5000)
			  TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_realestateagent', ('realestateagent'), amount)
			  Citizen.Wait(100)
